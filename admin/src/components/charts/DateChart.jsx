...
const DateChart = ({ chartData }) => {
  ...
  useEffect(() => {
    if (Array.isArray(chartData) && chartData.length > 0) {
      ...
      const values = sortedData.map(entry => entry.count);

      setData({
        labels,
        datasets: [
          {
            label: "User registration per Day",
            data: values,
  ...
  return (
    <div className="p-12 bg-white shadow rounded-xl w-full mx-auto" style={{ height: "40rem" }}>
      <h2 className="text-lg lg:text-2xl font-semibold mb-4 text-center">📈 User registration Over Time</h2>
      {data ? (
